...
public class List3<T> extends ListSecondary<T> {
    ...
    public final void addRightFront(T x) {
        ...
        //Make a node for what we want to add.
        Node tempToAdd = new Node();
        tempToAdd.data = x;

        //Get a reference to where we want to add.
        Node tempSpot = this.lastLeft.next;

        //Add in the node by fixing pointers and increment right length.
        tempToAdd.previous = this.lastLeft;
        tempToAdd.next = tempSpot;
        this.lastLeft.next = tempToAdd;
        tempSpot.previous = tempToAdd;
        this.rightLength++;
    ...
    public final T removeRightFront() {
        ...
        //Set up node pointing to what we want to remove and grab it's data.
        Node temp = this.lastLeft.next;
        T data = temp.data;
        this.lastLeft.next = temp.next;
        temp.next.previous = this.lastLeft;
