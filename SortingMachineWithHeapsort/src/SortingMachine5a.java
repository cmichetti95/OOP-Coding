...
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void siftDown(Array<T> array, int top, int last,
        ...
        if (array.length() > 1) {
            if (right <= last) {
                ...
                if (order.compare(array.entry(top), array.entry(lower)) > 1) {
                    array.exchangeEntries(top, lower);
                    siftDown(array, lower, last, order);
                }
            } else if (left == last) {
    ...
    public final T removeFirst() {
        ...
        T removed = this.heap.replaceEntry(0,
                this.heap.entry(this.heapSize - 1));
